...
train_image_dir = os.path.join(input_dir, 'train_v2')
test_image_dir = os.path.join(input_dir, 'test_v2')
...
masks = masks.drop(masks[masks.EncodedPixels.isnull()].sample(70000,random_state=50).index)
...
unique_img_index = masks.groupby('ImageId').size().reset_index(name='count')
train_index, valid_index = train_test_split(unique_img_index, 
                 test_size = 0.05, 
                 stratify = unique_img_index['count'],
                 random_state=50
                )
tr_df = pd.merge(masks, train_index)
val_df = pd.merge(masks, valid_index)
print(tr_df.shape[0], 'training masks')
print(val_df.shape[0], 'validation masks')


# In[47]:


tr_df['count'] = tr_df.apply(lambda col: col['count'] if 
                                    isinstance(col['EncodedPixels'], str) else
                                    0, 1)
val_df['count'] = val_df.apply(lambda col: col['count'] if 
                                    isinstance(col['EncodedPixels'], str) else
                                    0, 1)
...
tr_df['count'].hist(bins = 15)


# In[49]:


val_df['count'].hist(bins = 15)


# In[50]:


print(tr_df.ImageId.nunique(), 'unique images in train set')


# In[51]:


print(val_df.ImageId.nunique(),'unique images in test set')
...
class LossBinary:
   ...
   def __call__(self, outputs, targets):
       ...
       if self.weight:
           ...
           loss -= self.weight * torch.log((intersection + eps) / (union - intersection + eps))
       return loss
...
def validation(model: nn.Module, criterion, valid_loader):
    ...
    loss = {'valid_loss': valid_loss, 'jaccard_loss': valid_jaccard}
    return loss
